...
function useCheckoutForm() {
    ...
    info_email.innerHTML = `${signinUser.email}`;
    
    let info_name = document.querySelector(".personal-info-item.name div")
    info_name.innerHTML = `${signinUser.name}`;
    ...
    btn_buy.addEventListener("click",function () {
        ...
        if (isTrueValue) {
            ...
            thankTo.innerText = `Cảm ơn ${signinUser.name}`;
            thankOnDate.innerText = `Đơn hàng của bạn đã được đặt vào ngày ${strDate}`;
...
btn_orders_list.onclick = function () {
    ...
    renderOrderList(getLocaldata('orders'),getLocaldata('signinUser'));
}
...
function renderDetails(order_detail_id) {
    ...
    let order_title_date = order_detail.querySelector(".order-detail-date");
    order_title_date.innerText = `${info_order.date}`;
...
function renderOrderList(orders,signinUser) {
    let user_orders = [];
    orders.forEach(order => {
        if (order.email == signinUser.email) {
    ...
    user_orders.forEach(order => {
        ...
        order_date.innerText =`${order.date}`;
