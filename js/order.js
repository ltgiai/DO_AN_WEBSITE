...
function render(products) {
    ...
    categories.forEach(category => {
        ...
        if (category.getAttribute("catslug") == "cream") {
            let contentCategory = category.querySelector(".list-product");
            contentCategory.innerHTML = productCream_HTML;
        }
...
function renderUser(logInUser){
    let usernames = document.querySelectorAll(".login-user-name");
...
renderUser(getLocaldata('logInUser'));
setCatslug(getLocaldata('products'));
render(getLocaldata('products'));
...
if (localStorage.getItem('product-choose-id') != undefined) {
    var infoProduct = getProductById(getLocaldata("product-choose-id"));
        ...
        popupForm.querySelector(".overlay").style.display = "block";
        useAddProductForm(addProductForm,initProductInCustomize(infoProduct));
    localStorage.removeItem('product-choose-id');
}
...
function checkForm() {
    ...
    var inputs = checkout_form.getElementsByTagName('input');
    var btn_buy = document.querySelector(".btn-buy");
    var isValid = true;
    
    // Lặp qua tất cả các trường nhập liệu
    for (var i = 0; i < inputs.length; i++) {
        // Kiểm tra nếu trường nhập liệu không có giá trị
    ...
    // Hiển thị hoặc ẩn nút Submit tùy thuộc vào giá trị của biến isValid
    if (isValid) {
...
function useCheckoutForm() {
    ...
    info_name.innerHTML = `${logInUser.username}`;
    ...
    let strTime = yyyy + '-' + mm + '-' + dd + " | " + hour + ":" + min;

    let input_phone = document.getElementById("phone");
    input_phone.oninput = function () {
        var regex = /^0[1-9]\d{8}$/;

        if (!regex.test(input_phone.value)) {
            showError(input_phone,"Số điện thoại không hợp lệ")
        }else{
            showSuccess(input_phone)
        }
    }
    ...
    btn_buy.addEventListener("click",function () {
            ...
            thankTo.innerText = `Cảm ơn ${logInUser.username}`;
...
function useInputCheck(order,input_check,startDate,endDate) {
    if (!order.isDelivery && !order.isReceived && !order.isCancel) {
        input_check.classList.add("checkcancel");
        input_check.innerHTML = `
			<span>Huỷ Đơn Hàng</span>
			<input type="checkbox" name="user-cancel">
        ...
        input_check.onchange = function () {
            if (input_check.querySelector('input').checked) {
                ...
                renderOrderList(getLocaldata('orders'),getLocaldata('logInUser'),startDate,endDate);
            }
        }
    }

    if (order.isDelivery && !order.isReceived) {
        input_check.classList.add("checkreceived");
        input_check.innerHTML = `
			<span>Nhận Đơn Hàng</span>
			<input type="checkbox" name="user-receive">
			<span class="order-checkmark"></span>
        `;
        input_check.onclick = function () {
            input_check.querySelector('input').checked = true;
...
function filterOrderList(orders,startDate, endDate) {
    let filterOrders = [];
    orders.forEach(order => {
...
function renderOrderList(orders,logInUser,startDate,endDate) {
    let user_orders = [];
    orders.forEach(order => {
    ...
    let filter_orders = filterOrderList(user_orders,startDate,endDate);

    let orders_table = document.querySelector(".orders-list-area-content");
    orders_table.innerHTML = ``;
    filter_orders.forEach(order => {
        ...
        /* order_checkbox.innerHTML = `
			...
			<span class="order-checkmark"></span>
        `; */
        useInputCheck(order,order_checkbox,startDate,endDate);
        order_checkUser.appendChild(order_checkbox);
...
let btn_filter_orders = document.getElementById('btn-filter-orders');
btn_filter_orders.onclick = function () {
    let input_start_date = document.getElementById('filter-start-date');
    let input_end_date = document.getElementById('filter-end-date');
