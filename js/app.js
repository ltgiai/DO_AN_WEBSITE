...
let eyesSignIn = document.querySelectorAll(".sign-in-form .eye");
let eyesSignUp = document.querySelectorAll(".sign-up-form .eye");
...
function checkAgree(input) {
  let isAgreed = false;
  if (!input.checked) {
    alert("VUI LÒNG XÁC NHẬN ĐỒNG Ý VỚI CÁC ĐIỀU KHOẢN SỬ DỤNG");
  } else {
    isAgreed = true;
  }
...
function updateUI() {
  ...
  if (isUserLoggedIn()) {
      ...
      userContainer.innerHTML = `<p>${logInUser.username}!</p><button id="logout-btn">Đăng xuất</button>`;
...
function saveUserInfo(email, username, password, typeUser) {
  let users = getUsers();
  let isUsernameTaken = isUsernameRegistered(username);
  let isEmailTaken = isEmailRegistered(email);

  if (isUsernameTaken && isEmailTaken) {
      showError(SU_username, "Tên đăng nhập đã được đăng kí");
      showError(SU_email, "Email đã được đăng ký. Vui lòng chọn email khác.");
      return false;
  ...
  users.push({ email, username, password, typeUser });
...
SI_form.addEventListener("submit", function(e) {
  ...
  if (checkAdminCredentials(enteredUsername, enteredPassword, 'admin')) {
    ...
    if ( user  && user.password === enteredPassword) {
      ...
      let parentUsername = SI_username.parentElement;
      let smallUsername = parentUsername.querySelector("small");

      let parentPassword = SI_password.parentElement;
      let smallPassword = parentPassword.querySelector("small");

      if (!user) {
          parentUsername.classList.add("error");
          SI_username.classList.add("error");
          SI_username.classList.remove("success");
          smallUsername.innerText = "Tên đăng nhập không tồn tại.";
      } else {
          parentUsername.classList.remove("error");
          SI_username.classList.remove("error");
          SI_username.classList.add("success");
          ...
          smallPassword.innerText = "Sai mật khẩu.";
      }
  }
  }
});
...
function checkAdminCredentials(username, password, type) {
  // Lấy thông tin từ localStorage
  const users = JSON.parse(localStorage.getItem('users'));
  // Kiểm tra xem có thông tin nào trùng khớp với admin không
  const isAdmin = users && users.some(user => user.username === username && user.password === password && user.typeUser === type);
  return isAdmin;
}
//Valid Sign-Up
SU_form.addEventListener("submit",function(e){
  ...
  if (isAgreed && isCfPassword && isPassword && isEmail && isUsername) {
    if (saveUserInfo(SU_email.value ,SU_username.value, SU_password.value, 'user')) {
...
function resetForm() {
  SI_username.value = "";
  SI_username.classList.remove("active", "error", "success");

  SI_password.value = "";
  SI_password.classList.remove("active", "error", "success");
  SI_password.setAttribute("type", "password"); 
  eyesSignIn[0].innerHTML = `<i class="fas fa-eye-slash"></i>`;

  SU_username.value = "";
  SU_username.classList.remove("active", "error", "success");

  SU_email.value = "";
  SU_email.classList.remove("active", "error", "success");

  SU_password.value = "";
  SU_password.classList.remove("active", "error", "success");
  SU_password.setAttribute("type", "password"); 
  eyesSignUp[0].innerHTML = '<i class="fas fa-eye-slash"></i>';
  eyesSignUp[1].innerHTML = '<i class="fas fa-eye-slash"></i>';

  SU_confirmPass.value = "";
  SU_confirmPass.classList.remove("active", "error", "success");

  SU_agree.checked = false;

  resetInputStyles("sign-in-username");
  resetInputStyles("sign-in-password");
